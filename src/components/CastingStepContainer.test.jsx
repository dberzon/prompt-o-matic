...
vi.mock('./ActorBank/ActorBankView.jsx', () => ({
  default: ({ setActiveCharId, setActiveEntityId, setActiveBankSlug }) => (
    <button
      type="button"
      data-testid="mock-bank-char"
      onClick={() => {
        setActiveCharId?.('char_bank')
        setActiveEntityId?.(null)
        setActiveBankSlug?.('bank_slug')
      }}
    >
      Pick bank character
    </button>
...
describe('CastingStepContainer', () => {
  ...
  it('character selection from each sub-tab updates workflow state setters', () => {
    ...
    expect(setActiveBankSlug).toHaveBeenCalledWith('bank_slug')
  })
