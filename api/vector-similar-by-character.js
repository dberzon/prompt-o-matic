...
import { findSimilarCharactersById, parseSimilarByCharacterRequest } from './lib/vector/maintenance.js'
...
export default async function handler(req, res) {
  ...
  let runtime = null
  try {
    assertVectorOperationAllowed('similar-by-character', process.env)
    const body = req.body || {}
    ...
    return sendJsonNode(res, normalized.status, { error: normalized.message, code: error?.code || 'VECTOR_SIMILAR_BY_CHARACTER_ERROR' })
  } finally {
    runtime?.close?.()
  }
}
