...
export default async function handler(req, res) {
  ...
  let runtime = null
  try {
    assertGeneratedImagesOperationAllowed('approve', process.env)
    const body = req.body || {}
    ...
    return sendJsonNode(res, normalized.status, { error: normalized.message, code: error?.code || 'GENERATED_IMAGE_APPROVE_ERROR' })
  } finally {
    runtime?.close?.()
  }
}
