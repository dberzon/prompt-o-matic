...
| B2 | Six-stage extrapolation pipeline depth | Strong for **characters**: S1 canon → S2 cultural → S3 psychology → S4 environment → S5 visual → S6 conflict, with disk cache and parallel middle stages. **Weak for non-characters**: S3 hardcodes `behavior.* \| speech.* \| fear.*` prefixes (`s3Parser.js`); S5 is portrait-shaped (`s5VisualDescriptor.js`: "Prefer frontal portrait composition"). S4 only runs *from* a character; there is no inverse environment-enrichment track. | Major | High | Type-aware stage dispatch (extends A3): for `environment`, replace S3 with "Atmosphere & Mood Inference" (weather behavior, wear patterns, daily rhythm); for `era`, replace with "Material Culture Inference" (period objects, slang, fashion, taboos); etc. | 2 wk |
| B3 | Era Bible as a first-class type | **Absent.** Today "era" is at best a string-valued canon attribute (`setting.era`) that S2 keys off. No `era` entity type. | Major | High | Add `'era'` to the `entities.type` CHECK constraint (one-line schema migration); type-specific S1/S2/S3 prompts; bind characters/locations to one era via `entity_relationships` with `type='set_in_era'`. | 1 wk |
