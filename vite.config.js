...
import characterBankHandler from './api/character-bank.js'
...
import postMvpCapabilitiesHandler from './api/post-mvp-capabilities.js'
...
function apiDevPlugin(env) {
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use((req, res, next) => {
        const url = new URL(req.url || '', 'http://localhost')
        if (!/^\/api\/entities\/[^/]+\/attributes\/[^/]+\/(promote|dismiss|edit)\/?$/.test(url.pathname)) {
          next()
          return
        }
      ...
      server.middlewares.use((req, res, next) => {
        const url = new URL(req.url || '', 'http://localhost')
        if (!/^\/api\/entities\/[^/]+\/conflicts\/[^/]+\/(resolve|dismiss)\/?$/.test(url.pathname)) {
          next()
          return
        }
        void entityConflictActionsHandler(req, res)
      })
      ...
      server.middlewares.use((req, res, next) => {
        const url = new URL(req.url || '', 'http://localhost')
        if (!/^\/api\/post-mvp-capabilities\/?$/.test(url.pathname)) {
          next()
          return
        }
        void postMvpCapabilitiesHandler(req, res)
      })
      ...
      server.middlewares.use((req, res, next) => {
        const url = new URL(req.url || '', 'http://localhost')
        if (!/^\/api\/entities\/[^/]+\/extrapolate\/stage\/5\/?$/.test(url.pathname)) {
          next()
          return
        }
        void entityExtrapolateStage5Handler(req, res)
      })
      ...
      server.middlewares.use('/api/character-bank', async (req, res) => {
        void characterBankHandler(req, res)
      })
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [qpbDevServer({ env }), react(), apiDevPlugin(env)],
    test: {
